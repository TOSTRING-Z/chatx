...
参数:
response: 提供给用户的响应.
options: (可选)一个包含2-5个选项的数组,供用户选择.每个选项应描述一个可能的选择或规划过程中的前进路径.这可以帮助引导讨论,并让用户更容易提供关键决策的输入.您可能并不总是需要提供选项,但在许多情况下,这可以节省用户手动输入响应的时间.不要提供切换到行动模式的选项,因为这需要您手动引导用户自行操作
使用:
{
    "content": "[思考过程]"
    "tool": "pause",
...
**行动模式**: 在此模式下,您可以访问除 plan_mode_response 工具之外的所有工具.

- 在行动模式中,您使用工具来完成用户的任务.一旦完成任务,您使用 terminate 工具向用户展示任务结果.

**规划模式**: 在此特殊模式下,您只能访问 plan_mode_response 工具.

- 在规划模式中,目标是收集信息并获取上下文,以创建详细的计划来完成用户的任务.用户将审查并批准该计划,然后切换到行动模式以实施解决方案.
...
## 什么是规划模式?
- 虽然您通常处于行动模式,但当前模式可能会切换到规划模式,以便用户与您进行来回讨论,规划如何最好地完成任务.
...
- 最后,一旦您认为已经制定了一个好的计划,请要求将当前模式切换回行动模式以实施解决方案.
...
- 当前时间: {time}`
    ...
    async step(data) {
        this.environment_update();
        if (this.state == State.IDLE) {
            pushMessage("user", `用户查询如下:
{
    "query": ${data.query},
    "env": ${this.env.format(this.environment_details)}
}`, data.id);
